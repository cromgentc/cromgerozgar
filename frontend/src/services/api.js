...
export const api = {
  ...
  employerDashboard: (email = '') => apiRequest(`/dashboard/employer${email ? `?recruiterEmail=${encodeURIComponent(email)}` : ''}`),
  ...
  currentRecruiterPackage: (email) => apiRequest(`/recruiter-package-subscriptions/current?recruiterEmail=${encodeURIComponent(email)}`),
  activateRecruiterPackage: (data) => apiRequest('/recruiter-package-subscriptions/activate', { method: 'POST', body: JSON.stringify(data) }),
  purchaseRecruiterCoins: (data) => apiRequest('/recruiter-package-subscriptions/purchase-coins', { method: 'POST', body: JSON.stringify(data) }),
  submitRecruiterJob: (data) => apiRequest('/recruiter-job-posts/submit', { method: 'POST', body: JSON.stringify(data) }),
  list: (resource, params = '') => apiRequest(`/${resource}${params}`, { authRequired: resource === 'users' }),
  get: (resource, id) => apiRequest(`/${resource}/${id}`, { authRequired: resource === 'users' }),
