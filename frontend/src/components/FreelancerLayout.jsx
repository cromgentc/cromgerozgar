...
const freelancerNav = [
  ['Home', '/freelancer'],
  ['Projects Management', '/freelancer/projects'],
]
    ...
    <div className="min-h-screen bg-white text-slate-900">
      <header className="sticky top-0 z-50 border-b border-[#0057B8]/10 bg-white/92 shadow-sm shadow-[#0057B8]/10 backdrop-blur-xl">
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
          <Link className="site-logo-lockup flex min-w-0 items-center font-black text-slate-950" to="/freelancer">
            {branding.logoUrl ? (
          ...
          <div className="hidden items-center gap-1 lg:flex">
            {freelancerNav.map(([label, to]) => (
              <NavLink className="group relative rounded-[7px] px-3 py-2 text-sm font-bold text-slate-600 transition hover:text-[#0057B8]" key={label} to={to}>
                {label}
                <span className="absolute inset-x-3 -bottom-0.5 h-0.5 scale-x-0 rounded-[7px] bg-[#0057B8] transition group-hover:scale-x-100" />
              </NavLink>
            ))}
          </div>

          <div className="hidden items-center gap-2 lg:flex">
            <Link className="nav-transparent-btn" to={isFreelancerLoggedIn ? '/admin/projects' : '/'}>{isFreelancerLoggedIn ? 'Dashboard' : 'Main Site'}</Link>
            {!isFreelancerLoggedIn && (
              <>
                <Link className="nav-transparent-btn" to="/freelancer-login">Login</Link>
                <Link className="nav-post-job-btn inline-flex min-h-11 items-center justify-center rounded-[7px] bg-[#0057B8] px-5 py-2.5 text-sm font-black text-white" to="/freelancer-register">Register</Link>
              </>
            )}
          </div>
        ...
        <div className={`lg:hidden ${open ? 'pointer-events-auto' : 'pointer-events-none'}`}>
          ...
          <aside className={`fixed inset-y-0 right-0 z-[90] h-dvh w-80 max-w-[86vw] overflow-y-auto border-l border-[#0057B8]/10 bg-white px-4 py-4 shadow-2xl shadow-[#0057B8]/20 transition-transform duration-300 ease-out ${open ? 'translate-x-0' : 'translate-x-full'}`}>
            ...
            <div className="grid gap-2">
              {freelancerNav.map(([label, to]) => (
                <Link className="rounded-[7px] px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50" key={label} onClick={() => setOpen(false)} to={to}>
                  {label}
                </Link>
              ))}
              <div className="grid gap-2 pt-2">
                <Link className="nav-transparent-btn" onClick={() => setOpen(false)} to={isFreelancerLoggedIn ? '/admin/projects' : '/'}>{isFreelancerLoggedIn ? 'Dashboard' : 'Main Site'}</Link>
                {!isFreelancerLoggedIn && (
                  <>
                    <Link className="nav-transparent-btn" onClick={() => setOpen(false)} to="/freelancer-login">Login</Link>
                    <Link className="nav-post-job-btn inline-flex min-h-11 items-center justify-center rounded-[7px] bg-[#0057B8] px-5 py-2.5 text-sm font-black text-white" onClick={() => setOpen(false)} to="/freelancer-register">Register</Link>
                  </>
                )}
              </div>
            </div>
      ...
      <footer className="border-t border-[#0057B8]/10 bg-white">
        <div className="mx-auto grid max-w-7xl gap-3 px-3 py-5 text-xs font-semibold text-slate-500 sm:flex sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:py-6 sm:text-sm lg:px-8">
          ...
          <div className="flex flex-wrap gap-3 sm:gap-4">
            <Link className="hover:text-[#0057B8]" to="/freelancer/projects">Projects Management</Link>
            <Link className="hover:text-[#0057B8]" to="/support">Support</Link>
          </div>
        </div>
      </footer>
    </div>
