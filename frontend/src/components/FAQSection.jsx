...
const faqItems = [
  ...
  },
  {
    ...
    answer: 'Yes. Candidate registration is free, including profile creation, job search, saved jobs, resume upload, and application tracking features.',
  },
  {
    category: 'Candidate',
    question: 'Can I upload my resume?',
    answer: 'Yes. Candidates can upload a resume from the dashboard and use it while applying to premium job openings.',
  },
  {
  ...
  {
    category: 'Applications',
    question: 'Do you support remote and work-from-home jobs?',
    answer: 'Yes. The job listing filters support remote, hybrid, on-site, freelance, and work-from-home opportunities.',
  },
  {
    category: 'Payments',
...
export function FAQSection() {
  ...
  return (
    <section className="relative overflow-hidden bg-[radial-gradient(circle_at_top_left,#E0F2FE_0,transparent_34%),radial-gradient(circle_at_bottom_right,#F3E8FF_0,transparent_32%),linear-gradient(180deg,#FFFFFF_0%,#F8FAFC_100%)] py-16 sm:py-20">
      <div className="absolute left-1/2 top-20 h-64 w-64 -translate-x-1/2 rounded-full bg-teal-200/25 blur-3xl" />
      <div className="relative mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 rounded-[2rem] border border-white bg-white/75 p-4 shadow-xl shadow-blue-100/60 backdrop-blur-xl sm:p-5">
          ...
          <AnimatePresence mode="popLayout">
            {filteredFaqs.length > 0 ? (
              ...
              >
                ...
                <h3 className="mt-4 text-xl font-black text-slate-950">No questions found</h3>
                <p className="mt-2 text-sm text-slate-500">Try another keyword or switch FAQ category.</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
        ...
        <motion.div
          className="mt-8 rounded-[2rem] border border-blue-100 bg-white/80 p-5 text-center shadow-xl shadow-blue-100/50 backdrop-blur sm:p-6"
          initial={{ opacity: 0, y: 18 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.45 }}
    ...
    >
      ...
      >
        ...
        <div className="grid gap-3 bg-slate-50 p-5">
          <div className="max-w-[82%] rounded-2xl rounded-tl-md bg-white p-4 text-sm font-semibold leading-6 text-slate-600 shadow-sm">
            Hi, welcome to Cromgen Rozgar support. How can we help you today?
          </div>
          <div className="ml-auto max-w-[82%] rounded-2xl rounded-tr-md bg-blue-600 p-4 text-sm font-semibold leading-6 text-white shadow-sm">
            I need help with my account or applications.
          </div>
          <div className="max-w-[82%] rounded-2xl rounded-tl-md bg-white p-4 text-sm font-semibold leading-6 text-slate-600 shadow-sm">
            Sure. Share your question and our support team will guide you.
          </div>
        </div>
        ...
        <div className="flex gap-2 border-t border-slate-100 p-4">
          ...
          <button className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-blue-600 text-white shadow-lg shadow-blue-100 hover:bg-blue-700" type="button" aria-label="Send message">
            <Send size={18} />
          </button>
        </div>
      </motion.div>
...
function FAQCard({ item, isOpen, onClick }) {
  return (
    <motion.article
      className={`rounded-[1.6rem] bg-gradient-to-br p-px shadow-sm transition hover:-translate-y-0.5 hover:shadow-xl hover:shadow-blue-100 ${
        isOpen ? 'from-blue-500 via-teal-400 to-violet-500' : 'from-slate-200 to-slate-200'
      }`}
      layout
      ...
      <div className="rounded-[1.55rem] bg-white/95 p-4 backdrop-blur sm:p-5">
        <button className="flex w-full items-center gap-4 text-left" onClick={onClick} type="button">
          ...
          <span className="min-w-0 flex-1">
            <span className="mb-1 inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-black uppercase tracking-wide text-slate-500">
              {item.category}
            </span>
